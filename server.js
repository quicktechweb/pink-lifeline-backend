...
app.get('/api/user/me', verifyToken, async (req, res) => {
  ...
  try {
    const userDoc = await User.findOne({ userId: req.user.userId })
      .select('_id userId email type role')
      .lean();
...
app.get('/', async (req, res) => {
  res
    .status(200)
    .send(
      `🚀 Pink Lifeline Backend is running smoothly | STATUS: ONLINE | ⏰ ${new Date().toISOString()} | ⚡ Uptime: ${Math.floor(process.uptime())}s`
    );
});
...
app.listen(PORT, () => {
  ...
  console.log(
    `Server is running on ${ENV === 'dev' ? 'DEVELOPMENT' : 'PRODUCTION'} mode`
  );
  if (ENV === 'prod') {
