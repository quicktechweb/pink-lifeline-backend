...
internalUtilRoutes.put(
  '/update-property-by-id/:collectionName/:id',
  async (req, res) => {
    try {
...
internalUtilRoutes.post(
  '/delete-specific-data-form-each-collection',
  async (req, res) => {
    try {
...
internalUtilRoutes.patch('/update-schedule-time', async (req, res) => {
  try {
    const { userId, fcmTokens, body, title, type, autoReminderLimit } =
      req.body;
