...
const BLEEDING = {
  ...
  modFlow: {
    _id: '6a11766eb9dec79489ffc050',
    title: 'Moderate Flow',
    flowLevel: 2,
  },
  highFlow: {
    _id: '6a117675b9dec79489ffc053',
    title: 'High Flow',
    flowLevel: 3,
  },
};
...
async function testPeriodStart(dateStr, label) {
  ...
  record(
    `${label} → recordPeriodStart (${dateStr})`,
    pass ? 'PASS' : 'FAIL',
    res
  );
  return pass;
...
async function testPeriodCurrent(dateStr, dayIndex, totalDays, cycleLabel) {
  ...
  record(
    `${cycleLabel} → recordPeriodCurrent day +${dayIndex} (${dateStr})`,
    pass ? 'PASS' : 'FAIL',
    res
  );
}
...
async function testPeriodEnd(dateStr, cycleLabel) {
  ...
  const pass = res.ok || res.status === 200;
  record(
    `${cycleLabel} → recordPeriodEnd (${dateStr})`,
    pass ? 'PASS' : 'FAIL',
    res
  );
}
...
async function runEdgeCases() {
  ...
  // a) Future date on start
  {
    ...
    const res = await post(API_START, body);
    record(
      'EDGE: Start with future currentDate (expect 400)',
  ...
  // b) Mismatched currentDate vs startDate
  {
    ...
    const res = await post(API_START, body);
    record(
      'EDGE: currentDate ≠ startDate (expect 400)',
      res.status === 400 ? 'PASS' : 'FAIL',
      res
    );
  }
  ...
  // c) Missing userId
  {
    ...
    const res = await post(API_START, body);
    record(
      'EDGE: Missing userId on start (expect 400/404)',
      [400, 404].includes(res.status) ? 'PASS' : 'FAIL',
      res
    );
  }
  ...
  // d) Invalid flowLevel
  {
    ...
    const res = await post(API_START, body);
    record(
      'EDGE: Invalid flowLevel=99 (expect 400)',
      res.status === 400 ? 'PASS' : 'FAIL',
      res
    );
  }
  ...
  // e) recordPeriodCurrent with a date outside any active range
  {
    ...
    const res = await post(API_CURRENT, body);
    record(
      'EDGE: recordPeriodCurrent outside active range (expect 400)',
      res.status === 400 ? 'PASS' : 'FAIL',
      res
    );
  }
  ...
  // f) recordPeriodEnd with endDate < startDate
  {
    ...
    const res = await post(API_END, body);
    record(
      'EDGE: recordPeriodEnd with endDate before startDate (expect 400)',
      res.status === 400 ? 'PASS' : 'FAIL',
      res
    );
  }
  ...
  // g) recordPeriodEnd with missing endDate
  {
    ...
    const res = await post(API_END, body);
    record(
      'EDGE: recordPeriodEnd missing endDate (expect 400)',
      res.status === 400 ? 'PASS' : 'FAIL',
      res
    );
  }
  ...
  // h) recordPeriodEnd with mismatched currentDate ≠ endDate
  {
    ...
    const res = await post(API_END, body);
    record(
      'EDGE: recordPeriodEnd currentDate ≠ endDate (expect 400)',
      res.status === 400 ? 'PASS' : 'FAIL',
      res
    );
  }
  ...
  // i) No bleeding/spotting/symptoms in period body
  {
    ...
    const res = await post(API_START, body);
    record(
      'EDGE: Start with empty period body (expect 400)',
      res.status === 400 ? 'PASS' : 'FAIL',
      res
    );
  }
  ...
  // j) Providing both startDate AND endDate together
  {
    ...
    const res = await post(API_START, body);
    record(
      'EDGE: startDate + endDate together (expect 400)',
      res.status === 400 ? 'PASS' : 'FAIL',
      res
    );
  }
...
async function runAll() {
  ...
  for (const cycle of orderedCycles) {
    ...
    if (!started) {
      ...
      record(
        `${label} → recordPeriodCurrent SKIPPED (start failed)`,
        'SKIP',
        null
      );
    ...
    for (let day = 1; day < periodDays - 1; day++) {
      ...
      if (new Date(dayDateStr + 'T12:00:00.000Z') > TODAY) {
        record(
          `${label} → day +${day} (${dayDateStr}) SKIPPED — future date`,
          'SKIP',
          null
        );
        break;
    ...
    if (new Date(endDateStr + 'T12:00:00.000Z') > TODAY) {
      record(
        `${label} → recordPeriodEnd SKIPPED — end date is in future`,
        'SKIP',
        null
      );
    } else {
